...
- [x] 1.1 Replace the hero availability pill with a semantic status panel containing “Open to full-stack software engineering roles in Canada,” an interview-availability note, and a direct Contact action; verify the complete message and link remain visible with CSS animation and JavaScript disabled.
- [x] 1.2 Add a compact textual `Open to work` link to the desktop navigation and an appropriate simplified mobile treatment; verify it targets `#contact`, remains keyboard-focusable, and communicates meaning without its status color or pulse.
- [ ] 1.3 Style the hero and navigation indicators as restrained radar/orbit beacons in both themes and stop their motion under reduced motion; verify they are prominent without covering hero copy, navigation, or controls at 320px, tablet, and 1440px.
...
- [ ] 4.1 Confirm existing terminal, WhatBroke, experience, project, About, Contact, theme, and motion-tier behavior is unchanged outside the focused visual additions; verify all existing interactive controls still work by keyboard.
- [x] 4.2 Run `node --check script.js` and audit duplicate IDs, internal anchors, decorative accessibility attributes, obsolete availability markup, and added dependencies; verify syntax passes, targets resolve uniquely, decorative layers are hidden, and no new dependency exists.
- [ ] 4.3 Perform dark/light visual checks at 320px, tablet, and 1440px plus reduced-motion, no-JavaScript, canvas-blocked, and GSAP-blocked checks; verify every scenario in `specs/portfolio-presentation/spec.md` passes and explicitly confirm the hero, Skills, and availability qualitative review questions from `design.md`.
