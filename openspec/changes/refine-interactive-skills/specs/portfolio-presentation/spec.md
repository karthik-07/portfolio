# Spec Delta

## MODIFIED Requirements

### Requirement: Singular signature interaction
The portfolio SHALL retain the interactive hero terminal and a restrained ambient space treatment built from stars, orbital geometry, and lower-page horizon forms while omitting the broad nebula ribbons and oversized hero planet that compete with content.

#### Scenario: Visitor ignores the terminal
- **WHEN** a visitor does not interact with the terminal
- **THEN** all hiring-critical information remains available in standard page content

#### Scenario: Visitor views the ambient scene
- **WHEN** the portfolio is displayed in either theme
- **THEN** stars, orbital geometry, and horizon forms provide the space identity without broad blurred nebula ribbons or an oversized planet obscuring the hero

#### Scenario: Visitor watches the capable desktop scene
- **WHEN** animated canvas ambience is enabled on a viewport that supports celestial events
- **THEN** one shooting star appears after an initial randomized delay of approximately 4–8 seconds and subsequent shooting stars appear at randomized intervals of approximately 6–12 seconds without more than one active shooting star

#### Scenario: Motion reduction is requested
- **WHEN** the visitor enables reduced-motion preferences
- **THEN** non-essential ambient and transition motion is disabled or reduced without hiding content

### Requirement: Restrained interaction model
The portfolio SHALL omit the command palette, pointer-following glow or trail, click ripples, hidden constellations, and magnetic controls while allowing bounded three-dimensional tilt on established card surfaces, including Skills modules, only for capable fine-pointer devices without a reduced-motion request.

#### Scenario: Visitor points or clicks around the page
- **WHEN** pointer movement or a click occurs outside an explicit control or supported card surface
- **THEN** the page does not create a pointer-following effect, ripple, hidden discovery, magnetic displacement, or unrelated card response

#### Scenario: Fine-pointer visitor explores a Skills module
- **WHEN** a visitor using a capable fine pointer moves across a Skills module while full motion is enabled
- **THEN** that module responds with the same bounded pointer-relative tilt and lift language used by the Professional Experience cards while its related decorative connection is emphasized

#### Scenario: Visitor selects a Skills module
- **WHEN** a visitor hovers, keyboard-focuses, or touch-selects a Skills module
- **THEN** that module and its related decorative connection receive a clear bounded emphasis without hiding other content or being required to read its technologies

#### Scenario: Motion or pointer capability is limited
- **WHEN** reduced motion is requested, the motion tier is constrained or static, or the primary input cannot hover precisely
- **THEN** Skills modules remain static while preserving visible hover where applicable, focus, selection, and readable content

### Requirement: Visible experience and capabilities
The portfolio SHALL expose every professional role and each technical capability group in the document without requiring tab selection or scripted interaction, and SHALL arrange Skills modules on visibly aligned desktop, tablet, and mobile layouts.

#### Scenario: JavaScript is unavailable
- **WHEN** the page is loaded without JavaScript
- **THEN** all experience entries and technical capability groups remain visible and readable in their default aligned arrangement

#### Scenario: Recruiter scans experience and skills
- **WHEN** the recruiter scrolls through the experience and capabilities sections
- **THEN** roles, dates, organizations, evidence bullets, capability categories, and technologies can be compared without switching views

#### Scenario: Recruiter scans Skills on desktop
- **WHEN** the Skills section is displayed on a desktop viewport
- **THEN** all six modules use deliberate shared edges, consistent gaps, and connector endpoints aligned to their modules while AWS & DevOps and AI-Assisted Development remain visually emphasized

#### Scenario: Visitor views Skills on tablet or mobile
- **WHEN** the viewport no longer safely supports the desktop constellation
- **THEN** Skills reflows to a balanced two-column tablet layout and a single-column mobile layout with no clipped text, overlap, or horizontal page overflow

## ADDED Requirements

### Requirement: Concise Skills introduction
The Skills heading SHALL stand on its own without the explanatory sentence “Six groups, each built around the kind of problem it solves.” or replacement filler copy.

#### Scenario: Recruiter enters the Skills section
- **WHEN** the Skills section heading is displayed
- **THEN** the heading is followed directly by the Skills composition without the removed explanatory sentence
